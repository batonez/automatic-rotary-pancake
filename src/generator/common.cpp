...
static inline void ensure_steepness_sanity(Vector2i &result)
{
  ...
  if (result.x < 0 && -result.x > result.y) {
    GENERATOR_LOG("S Warning: steepness is %d %d, making it sane", result.x, result.y);
    result.x = -result.y;
  }
  ...
  bool exact_destination)
{
  ...
  GENERATOR_LOG("S %d %d", result.x, result.y);
  ...
  if (!from_height || !to_height) {
    GENERATOR_LOG("S No destination height/width, so won't alter steepness any more");
  ...
  GENERATOR_LOG("S maxFinalHeight = %d = %d + %d * %d", maxFinalHeight, from_height, colsLeft, max_steepness);
  GENERATOR_LOG("S minFinalHeight = %d = %d - %d * %d", minFinalHeight, from_height, colsLeft, max_steepness);
  GENERATOR_LOG("S maxRequiredFinalHeight = %d = %d + %d", maxRequiredFinalHeight, to_height, max_steepness); // fixme but not greater than area height
  GENERATOR_LOG("S minRequiredFinalHeight = %d = %d - %d", minRequiredFinalHeight, to_height, max_steepness); // fixme but not less than zero
  
  if (maxFinalHeight <=  maxRequiredFinalHeight) {
    GENERATOR_LOG("S Forcing steepness up");
    result.x = -result.y;
  } else if (minFinalHeight >= minRequiredFinalHeight) {
    GENERATOR_LOG("S Forcing steepness down");
    result.y = -result.x;
  }
